...
namespace Kwiztime
{
    public static class BotRegistry
    {
        private static readonly List<BotDefinition> _bots = new()
        {
            ...
            // FIX: Legend Bot accuracy capped at 0.90f per GDD (was 0.92 and 0.95)
            new BotDefinition { botId="bot_kumo",  displayName="Kumo",  personality=BotPersonality.RareBoss, accuracy=0.90f, minDelay=0.9f, maxDelay=2.0f, mascotId=4, isRare=true, bonusCoins=50 },
            new BotDefinition { botId="bot_reina", displayName="Reina", personality=BotPersonality.RareBoss, accuracy=0.90f, minDelay=0.8f, maxDelay=1.8f, mascotId=5, isRare=true, bonusCoins=75 },
        };

        // FIX: replaced sequential _cursor with random selection to prevent predictable bot order
        public static BotDefinition GetRandomNonRare()
        {
            var pool = new List<BotDefinition>();
            for (int i = 0; i < _bots.Count; i++)
                if (!_bots[i].isRare) pool.Add(_bots[i]);

            if (pool.Count == 0) return null;
            return pool[UnityEngine.Random.Range(0, pool.Count)];
        }

        public static BotDefinition GetRandomRare()
        {
            var pool = new List<BotDefinition>();
            for (int i = 0; i < _bots.Count; i++)
                if (_bots[i].isRare) pool.Add(_bots[i]);
